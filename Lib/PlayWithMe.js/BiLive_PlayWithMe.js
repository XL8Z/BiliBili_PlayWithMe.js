class BiLive_PlayWithMeJS {
    ...
    /**
     * 礼物合并延迟
     * - 如默认3，则超过3秒没有再送相同礼物视为连击结束
     * - 礼物延迟播报的罪魁祸首
     * - 过短则无法正确合并礼物连击
     */
    static GiftCombine_Countdown = 3;
...
class BiLive_PlayWithMeJS_WEBSocketClient extends WebSocket {
    ...
    HeartBeat() {
        ...
        console.log("[BiLive_PlayWithMeJS] [开平长链]", "发送心跳包【踢服务器一jio】");
        console.debug("[BiLive_PlayWithMeJS] [开平长链]", "发送\n" + BiLive_PlayWithMeJS_UtilTools.Uint8Array2HexStr(PkgData));
        // 送走，收工
        this.send(PkgData);
    }
        ...
        if (Pkg[7] < 2) {
            switch (Pkg[11]) {
                ...
                case 5:
                    ...
                    console.log("[BiLive_PlayWithMeJS] [开平长链]", "收到新事件");
                    console.debug(Jsn);
                    // 进行事件处理
...
class BiLive_PlayWithMeJS_Authorizer {
    ...
    static LocalAuthorizer_GenerateAuthorizedHead(RequestBody) {
        ...
        console.debug("[BiLive_PlayWithMeJS] [开平签名器] [本地模式]", "对上述内容使用你的开发者SecretKey进行HMAC-SHA256签名得到的结果为\n" + HMAC);
        return {
